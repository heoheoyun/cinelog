...
public class ReviewController {
	...
	// 리뷰 수정 폼 (본인 리뷰만 접근 가능)
	@GetMapping("/review/modify")
	public String modifyForm(Long rno, Model model, HttpSession session) {
	...
	public String modify(Long rno, int score, String content, HttpSession session) {
		ReviewEntity review = reviewService.findById(rno);
		if (!isOwner(session, review))
			return "redirect:/movie/detail?mno=" + review.getMovie().getMno();
	...
	public String delete(Long rno, HttpSession session) {
		ReviewEntity review = reviewService.findById(rno);
